...
///
/// Preserves the lexicon's `required` declaration order (deduplicated) so that
/// generated typestate machinery is deterministic across runs. (Catbird patch:
/// the upstream 0.9.5 implementation round-tripped through a `HashSet`, which
/// emitted the builder associated types in a different order on every run.)
pub fn collect_required_fields(schema: &super::BuilderSchema<'_>) -> Vec<RequiredField> {
    ...
    let mut seen: HashSet<RequiredField> = HashSet::new();
    ...
    for field_name in required {
        ...
        let field = RequiredField::new(field_name_str);
