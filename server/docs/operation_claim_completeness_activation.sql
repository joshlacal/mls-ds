...
BEGIN
    SELECT count(*)
      INTO classified_legacy_count
      FROM chat.idempotency_records
     WHERE NOT operation_claim_required;

    SELECT legacy_receipt_count
      INTO STRICT recorded_legacy_count
      FROM chat.operation_claim_completeness_cutover
     WHERE singleton;

    IF classified_legacy_count <> recorded_legacy_count THEN
        RAISE EXCEPTION
            'operation-claim legacy classification count mismatch'
            USING ERRCODE = '23514';
    END IF;
