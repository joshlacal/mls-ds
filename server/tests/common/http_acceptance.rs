...
impl FromRef<TestState> for DbPool {
    fn from_ref(state: &TestState) -> Self { state.pool.clone() }
}
impl FromRef<TestState> for Arc<ChatRuntime> {
    fn from_ref(state: &TestState) -> Self { state.runtime.clone() }
}
impl FromRef<TestState> for BlobStore {
    fn from_ref(state: &TestState) -> Self { state.blob_store.clone() }
}
...
pub fn random_did() -> String {
    let bytes = [Uuid::new_v4().as_bytes().to_vec(), Uuid::new_v4().as_bytes().to_vec()].concat();
    let suffix: String = bytes.iter().take(24).map(|b| b"abcdefghijklmnopqrstuvwxyz234567"[(*b as usize) % 32] as char).collect();
    format!("did:plc:{suffix}")
}

pub fn random_p256() -> SigningKey {
    loop {
        let seed = [Uuid::new_v4().as_bytes().to_vec(), Uuid::new_v4().as_bytes().to_vec()].concat();
        if let Ok(key) = SigningKey::from_slice(&seed) { return key; }
    }
}
...
fn nest_key() -> SigningKey { SigningKey::from_bytes((&[0x5a_u8; 32]).into()).expect("nest key") }
...
fn dpop_proof(key: &SigningKey, jwk: &Value, method: &str, htu: &str, token: &str, now: i64) -> String {
    sign_jwt(
...
fn htu(nsid: &str) -> String { format!("{EXTERNAL_BASE}/xrpc/{nsid}") }
...
pub fn seed_device(pool: &DbPool) -> impl Future<Output = Device> + '_ {
    async move {
        ...
        let key = [Uuid::new_v4().as_bytes().to_vec(), Uuid::new_v4().as_bytes().to_vec()].concat();
        let now: chrono::DateTime<chrono::Utc> = sqlx::query_scalar("SELECT clock_timestamp()").fetch_one(pool).await.expect("clock");
        sqlx::query("INSERT INTO chat.principals(user_did,created_at) VALUES($1,$2)").bind(&did).bind(now).execute(pool).await.expect("principal");
        let key_id: String = sqlx::query_scalar("SELECT chat.ed25519_key_id($1)").bind(&key).fetch_one(pool).await.expect("key id");
        ...
        Device { did, device_id, signing, jwk, jkt }
    }
}

pub async fn ensure_fence(pool: &DbPool) {
    let key: String = sqlx::query_scalar("SELECT chat.ed25519_key_id($1)").bind(vec![0x51_u8; 32]).fetch_one(pool).await.expect("cursor key");
    ...
    let id: Uuid = sqlx::query_scalar("SELECT protocol_instance_id FROM chat.protocol_instances WHERE singleton=TRUE").fetch_one(pool).await.expect("instance");
...
pub async fn router(pool: DbPool) -> Router {
    let key_id: String = sqlx::query_scalar("SELECT cursor_key_id FROM chat.protocol_instances WHERE singleton=TRUE").fetch_one(&pool).await.expect("cursor key");
    ...
    ENV.call_once(|| {
        ...
        std::env::set_var("CHAT_SUBSCRIPTION_ENDPOINT", "wss://chat.example.net/xrpc/blue.catbird.chat.subscribeEvents");
    });
    ...
    chat_router::<TestState>().with_state(TestState { pool, runtime, blob_store: BlobStore::for_route_tests() })
}
...
pub fn unsigned_request_as(
    ...
    Request::builder().method(method).uri(format!("/xrpc/{nsid}{query}")).header("authorization", format!("DPoP {token}")).header("dpop", proof).body(Body::empty()).expect("request")
}
pub fn unsigned_json_request(
    device: &Device,
    nsid: &str,
    body: Vec<u8>,
) -> Request<Body> {
    ...
    let proof = dpop_proof(&device.signing, &device.jwk, "POST", &htu(nsid), &token, now);
    Request::builder()
        .method("POST")
