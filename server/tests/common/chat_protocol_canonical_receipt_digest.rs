...
use super::*;
use crate::chat_protocol::{cursor::CursorSealer, repository::{subscription, ticket}};
use sqlx::Row;

#[path = "/tmp/mlsv2-open-due-recovery-20260905/inventory-generation-source/mls-ds/server/src/chat_protocol/repository/subscription.rs"]
mod legacy_producer;
...
async fn canonical_replay_preserves_the_exact_legacy_receipt_and_rejects_changed_inputs() {
    ...
    let consumed = ticket::consume_subscription_ticket(
        &mut tx, &ticket::ticket_hash(&raw_ticket), &inventory.cursor,
        ticket::SUBSCRIBE_EVENTS_PATH, now,
    ).await.expect("actual HTTP-minted one-use ticket consumption");
    subscription::ensure_initial_receipt(&mut tx, &consumed).await.unwrap();
    tx.commit().await.unwrap();
    let key: [u8; 32] = URL_SAFE_NO_PAD.decode(&consumed.cursor_key_id).unwrap().try_into().unwrap();
    ...
    let mut tx = pool.begin().await.unwrap();
    let through = subscription::replay_high_water(&mut tx, &consumed).await.unwrap();
    let events = subscription::visible_events(&mut tx, &consumed, consumed.event_position, through, 10)
        .await.unwrap();
    assert_eq!(events.len(), 1, "one actual signed addressed application event");
    ...
    let mut tx = pool.begin().await.unwrap();
    subscription::replay_high_water(&mut tx, &consumed).await.unwrap();
    sqlx::query("SAVEPOINT unpublished_digest_probe").execute(&mut *tx).await.unwrap();
    let (_, _, probe_hash) = legacy_producer::materialize_envelope(
        &mut tx, &consumed, old_event(&event), &inventory.cursor, consumed.event_cursor_hash, &sealer,
    ).await.unwrap();
    let row = sqlx::query("SELECT * FROM chat.event_cursor_receipts WHERE cursor_hash=$1")
        .bind(probe_hash.as_slice()).fetch_one(&mut *tx).await.unwrap();
    let invalid = ticket::NewEventCursorReceipt {
        cursor_hash: probe_hash,
        inventory_session_id: consumed.inventory_session_id,
        user_did: consumed.user_did.clone(), device_id: consumed.device_id,
        jkt: consumed.jkt.clone(), auth_generation: consumed.auth_generation,
        protocol_instance_id: consumed.protocol_instance_id,
        cursor_key_id: consumed.cursor_key_id.clone(), event_position: event.event_position,
    ...
    sqlx::query("ROLLBACK TO SAVEPOINT unpublished_digest_probe").execute(&mut *tx).await.unwrap();
    ticket::insert_event_cursor_receipt(&mut tx, &invalid).await.unwrap();
    assert!(matches!(subscription::materialize_envelope(
        &mut tx, &consumed, event.clone(), &inventory.cursor, consumed.event_cursor_hash, &sealer,
    ).await, Err(ticket::TicketRepositoryError::InvalidReceipt)));
    ...
    let parent_before = row_by_session(&pool, consumed.inventory_session_id).await.unwrap();

    let mut tx = pool.begin().await.unwrap();
    subscription::replay_high_water(&mut tx, &consumed).await.unwrap();
    let (canonical, replay_cursor, replay_hash) = subscription::materialize_envelope(
        &mut tx, &consumed, event.clone(), &inventory.cursor, consumed.event_cursor_hash, &sealer,
    ).await.expect("exact historical producer digest remains readable");
    tx.commit().await.unwrap();
    ...
    assert_eq!(cursor, replay_cursor); assert_eq!(hash, replay_hash);
    assert_eq!(canonical_json["payload"], legacy_json["payload"]);
    assert_eq!(canonical_json["previousCursor"], legacy_json["previousCursor"]);
    assert_eq!(receipt(&pool, &cursor).await.unwrap(), before, "all retained receipt bytes unchanged");
    ...
    for changed in ["payload", "date", "cursor", "predecessor"] {
        ...
        subscription::replay_high_water(&mut tx, &consumed).await.unwrap();
        assert!(matches!(subscription::materialize_envelope(
            &mut tx, &consumed, altered, &previous, predecessor, &sealer,
        ).await, Err(ticket::TicketRepositoryError::InvalidReceipt)), "changed {changed} must reject");
        tx.rollback().await.unwrap();
    }
    assert_eq!(receipt(&pool, &cursor).await.unwrap(), before);
    assert_eq!(row_by_session(&pool, consumed.inventory_session_id).await.unwrap(), parent_before);
}
