...
fn genesis_group_info_rejects_excessive_leaf_lifetime_as_declared_error() {
    let fixture_raw = std::fs::read_to_string("tests/fixtures/captured_create_conversation_request.json")
        .expect("read captured fixture");
    let fixture: Value = serde_json::from_str(&fixture_raw).expect("parse fixture json");

    let group_info_b64 = fixture["body"]["signedRequest"]["body"]["genesisGroupInfo"]["bytes"].as_str().unwrap();
    let group_info_bytes = STANDARD.decode(group_info_b64).unwrap();

    let user_did = fixture["body"]["signedRequest"]["body"]["actorDid"].as_str().unwrap();
    let device_id = fixture["body"]["signedRequest"]["body"]["actorDeviceId"].as_str().unwrap();
    let basic_credential = format!("{}#{}", user_did, device_id).into_bytes();

    let pubkey_b64 = fixture["body"]["signedRequest"]["body"]["metadataSnapshot"]["authorProof"]["signaturePublicKey"].as_str().unwrap();
...
fn validate_latest_captured_create_conversation() {
    if let Ok(fixture_raw) = std::fs::read_to_string("/tmp/last_blue.catbird.chat.createConversation_body.json") {
        let fixture: Value = serde_json::from_str(&fixture_raw).expect("parse fixture json");
        let group_info_b64 = fixture["signedRequest"]["body"]["genesisGroupInfo"]["bytes"].as_str().unwrap();
        let group_info_bytes = STANDARD.decode(group_info_b64).unwrap();

        let user_did = fixture["signedRequest"]["body"]["actorDid"].as_str().unwrap();
        let device_id = fixture["signedRequest"]["body"]["actorDeviceId"].as_str().unwrap();
        let basic_credential = format!("{}#{}", user_did, device_id).into_bytes();

        let pubkey_b64 = fixture["signedRequest"]["body"]["metadataSnapshot"]["authorProof"]["signaturePublicKey"].as_str().unwrap();
        let pubkey_bytes = STANDARD.decode(pubkey_b64).unwrap();
...
async fn replay_create_conversation_against_router() {
    ...
    if let Ok(fixture_raw) = std::fs::read_to_string("/tmp/last_blue.catbird.chat.createConversation_body.json") {
        let fixture: Value = serde_json::from_str(&fixture_raw).expect("parse fixture json");
        let actor_did = fixture["signedRequest"]["body"]["actorDid"].as_str().unwrap();
        let actor_device_id: uuid::Uuid = fixture["signedRequest"]["body"]["actorDeviceId"].as_str().unwrap().parse().unwrap();
        let actor_key_id = fixture["signedRequest"]["body"]["keyId"].as_str().unwrap();
        let pubkey_b64 = fixture["signedRequest"]["body"]["metadataSnapshot"]["authorProof"]["signaturePublicKey"].as_str().unwrap();
        let actor_pubkey = STANDARD.decode(pubkey_b64).unwrap();
        let signed_at_str = fixture["signedRequest"]["body"]["signedAt"].as_str().unwrap();
        let now: chrono::DateTime<chrono::Utc> = chrono::DateTime::parse_from_rfc3339(signed_at_str).unwrap().with_timezone(&chrono::Utc);
        ...
        let participants = fixture["signedRequest"]["body"]["manifest"]["participants"].as_array().unwrap();
        ...
        let header = serde_json::json!({"alg":"ES256","typ":"JWT","kid":format!("{actor_did}#atproto")});
        ...
        let c = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(serde_json::to_vec(&claims).unwrap());
        let input = format!("{h}.{c}");
        use p256::ecdsa::signature::Signer as _;
        let sig: p256::ecdsa::Signature = p256_key.sign(input.as_bytes());
        let bearer = format!("{input}.{}", base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(sig.to_bytes()));
        ...
        println!("Replay createConversation status: {:?}, body: {:?}", status, body);
