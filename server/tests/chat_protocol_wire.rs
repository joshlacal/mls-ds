...
fn sealed_crypto_wire_v09_corpus_consumer_validation() {
    ...
    #[cfg(feature = "test-support")]
    {
        ...
        fn decode_json_from_cbor_bytes(cbor_bytes: &[u8]) -> Vec<u8> {
            ...
            fn node_to_json(
                ...
                match node {
                    ...
                    CborNode::Bytes(b) => {
                        let is_id_bytes =
                            parent == Some("coordinate") && key == Some("conversationId");
                        if b.len() == 16
                            && !is_id_bytes
                            && key.is_some_and(|k| UUID_KEYS.contains(&k))
                        {
        ...
        let verified_creation = decode_and_verify_signed_mutation(&creation_raw_json, &alice_pub)
            .expect("verify creation");
        assert_eq!(verified_creation.kind(), SignedMutationKind::Creation);
        let VerifiedMutationProjection::Creation(creation_proj) = verified_creation.projection()
        else {
            panic!("expected Creation projection");
        };
        let creation_next = creation_proj.next();
        let CanonicalValueRef::Integer(creation_sv) = creation_next.get("stateVersion").unwrap()
        else {
            panic!("stateVersion");
        };
        ...
        let VerifiedMutationProjection::MetadataTransition(sv1_proj) = verified_sv1.projection()
        else {
            panic!("expected MetadataTransition projection");
        };
        let CanonicalValueRef::Integer(sv1_prior_sv) =
            sv1_proj.prior().get("stateVersion").unwrap()
        else {
            panic!("stateVersion");
        };
        ...
        let VerifiedMutationProjection::PolicyTransition(sv2_proj) = verified_sv2.projection()
        else {
            panic!("expected PolicyTransition projection");
        };
        let CanonicalValueRef::Integer(sv2_prior_sv) =
            sv2_proj.prior().get("stateVersion").unwrap()
        else {
            panic!("stateVersion");
        };
        ...
        let VerifiedMutationProjection::MetadataTransition(sv6_proj) = verified_sv6.projection()
        else {
            panic!("expected MetadataTransition projection");
        };
        let CanonicalValueRef::Integer(sv6_prior_sv) =
            sv6_proj.prior().get("stateVersion").unwrap()
        else {
            panic!("stateVersion");
        };
        ...
        let VerifiedMutationProjection::PolicyTransition(sv7_proj) = verified_sv7.projection()
        else {
            panic!("expected PolicyTransition projection");
        };
        let CanonicalValueRef::Integer(sv7_prior_sv) =
            sv7_proj.prior().get("stateVersion").unwrap()
        else {
            panic!("stateVersion");
        };
