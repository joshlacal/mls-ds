...
fn system_reset_did() -> &'static str {
    ...
    // N31: parsing/fail-loudly behavior lives in the shared
    // `identity::service_did_base` helper (fragment-stripped, panics when
    // SERVICE_DID is unset/empty) — tested in `identity::tests`.
    DID.get_or_init(crate::identity::service_did_base)
}
