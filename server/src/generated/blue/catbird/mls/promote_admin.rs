...
//!Definitions for the `blue.catbird.mls.promoteAdmin` namespace.
//!Promote a member to admin status (admin-only operation)
pub const NSID: &str = "blue.catbird.mls.promoteAdmin";
...
impl std::fmt::Display for Error {
    fn fmt(&self, _f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Error::AlreadyAdmin(msg) => {
                write!(_f, "AlreadyAdmin")?;
                if let Some(msg) = msg {
                    write!(_f, ": {msg}")?;
                }
            }
