...
// `repository::core` is deliberately excluded from path-included unit and
// integration test crates. This mirror keeps those tests isolated; production
// can receive the non-forgeable repository-owned guard above only.
#[cfg(test)]
...
impl AllocatedProjectionRevisionGuard {
    pub(crate) fn for_test(value: u64) -> Self {
        Self::for_test_allocation(Uuid::new_v4(), value)
    }

    pub(crate) fn for_test_allocation(allocation_id: Uuid, value: u64) -> Self {
        assert!((1..=MAX_PERSISTED_SAFE_INTEGER).contains(&value));
        assert!(uuid_is_canonical_v4(allocation_id));
        Self {
            allocation_id,
...
// Production load witnesses are minted only by the SQL repository while it
// holds the exact durable operation state. Path-included tests use these
// mirrors to exercise the same consuming hydration boundary without importing
...
impl RelationshipProjectionLoadGuard {
    ...
    fn into_parts(self) -> (ProjectionOperationScope, ProjectionScope) {
...
impl TrafficProjectionLoadGuard {
    ...
    fn into_scope(self) -> TrafficGraphScope {
        self.scope
    }
}

/// Test mirror of the repository-owned post-collection persistence clock.
/// Production callers can only obtain this opaque instant from the repository
...
impl TrustedRelationshipPersistenceInstant {
    pub(crate) fn for_test(observed_at: DateTime<Utc>) -> Self {
        Self(observed_at)
    }
...
/// Test mirror of the repository-owned post-lock decision instant. The real
/// value is non-cloneable and is minted only from one exact transaction-bound
/// locked business read-set.
#[cfg(test)]
...
impl TrustedRelationshipDecisionInstant {
    pub(crate) fn for_test_relationship(
        ...
        Self::from_locked_relationship_scope(
            transaction_id,
            operation_scope,
            scope,
        ...
        .expect("valid test relationship decision instant")
    }
    ...
    pub(crate) fn for_test_traffic(
        ...
        .expect("valid test traffic decision instant")
    }

    pub(crate) fn from_locked_relationship_scope(
        transaction_id: String,
        operation_scope: ProjectionOperationScope,
        scope: ProjectionScope,
        authenticated_actor_digest: [u8; 32],
...
#[cfg(test)]
fn test_transaction_id_is_canonical(value: &str) -> bool {
    !value.is_empty()
        && value.len() <= 20
