...
pub async fn warn_member(
    ...
    info!(
        "📍 [warn_member] START - actor: {}, convo: {}, target: {}, reason: {}",
        auth_user.did,
        input.convo_id,
        input.member_did.as_str(),
        input.reason
    );
    ...
    let expires_at = input
        .expires_at
        .as_ref()
        .map(|dt| crate::sqlx_atrium::datetime_to_chrono(dt));
    ...
    info!(
        "✅ [warn_member] SUCCESS - warning {} issued by {} to {} in convo {}",
        warning_id,
        auth_user.did,
        input.member_did.as_str(),
        input.convo_id
